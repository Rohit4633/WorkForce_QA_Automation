...
export class DocumentsPage extends BasePage {
  ...
  async scrollToDocuments() {
    // Try heading role first (any level); fall back to the section label text
    const heading = this.page.getByRole('heading', { name: 'Documents' }).first();
    const isVisible = await heading.isVisible().catch(() => false);
    if (isVisible) {
      await heading.scrollIntoViewIfNeeded();
    } else {
      // Fallback: scroll to the Documents section container text
      await this.page.getByText('Documents', { exact: true }).first().scrollIntoViewIfNeeded();
    }
  ...
  // ── Try to add passport — handles "already exists" error gracefully ───────
  async tryAddPassport(pn: string): Promise<'added' | 'already_exists'> {
    await this.clickAddDocument();
    await this.selectDocumentType('Passport');
    await this.fillPassportNumber(pn);
    await this.fillIssueDate('2020-01-15');
    await this.fillExpiryDate('2030-01-15');
    await this.selectCountry('India');
    await this.clickSave();
    await this.page.waitForTimeout(1000);

    // Check if "already exists" error appeared
    const alreadyExists = await this.page
      .getByText('passport already exists for user')
      .isVisible()
      .catch(() => false);

    if (alreadyExists) {
      console.log('⚠️ Passport already exists — cancelling modal');
