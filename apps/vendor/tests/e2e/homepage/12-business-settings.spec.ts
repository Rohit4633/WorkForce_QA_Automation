...
test.describe('Business Settings', () => {
  ...
  // ── BIZ-01: Business settings page shows company name and country ──────────
  test('BIZ-01: business settings page shows company name and country', async ({ page }) => {
    const settingsPage = new BusinessSettingsPage(page, test);
    ...
    await expect(page.getByText('1455 Noon Test AE')).toBeVisible();
    console.log('✅ Company name "1455 Noon Test AE" is visible');

    await expect(page.getByText('United Arab Emirates')).toBeVisible();
    console.log('✅ Country "United Arab Emirates" is visible');
  });
