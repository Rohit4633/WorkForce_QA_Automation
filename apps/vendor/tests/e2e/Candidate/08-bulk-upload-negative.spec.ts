...
test.describe('Candidate — Bulk Upload Negative Scenarios', () => {
  ...
  test('upload with missing mandatory fields shows error then fix and re-upload', async ({ page }) => {
    ...
    // Step 12 — Create fixed template (unique file name to avoid collision with parallel test)
    const fixedFilePath = createFixedTemplate('Fixed', 'Candidate', 'fixed-missing');
  ...
  test('upload duplicate candidate shows error then fix and re-upload', async ({ page }) => {
    ...
    // Step 13 — Create fixed template with unique Emirates ID (unique file name to avoid collision)
    const fixedFilePath = createFixedTemplate('Unique', 'Candidate', 'fixed-duplicate');
