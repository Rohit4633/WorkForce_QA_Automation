...
export class BulkUploadPage extends BasePage {
  ...
  async uploadFile(filePath: string) {
    ...
    if (inputCount > 0) {
      ...
      await this.page.getByText('Or choose file').click();
  ...
  async readErrorReport(filePath: string): Promise<string[]> {
    ...
    // Read the downloaded error report
    ...
    // Find Result column index from header row
    ...
    // Extract error messages — skip header row and example row
    const errors: string[] = [];
